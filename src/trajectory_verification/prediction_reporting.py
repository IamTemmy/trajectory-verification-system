...
def evaluation_to_markdown(evaluation: PredictionEvaluation) -> str:
    lines = [
        ...
        "",
        "| Scenario | Agents | Mean minADE | Mean minFDE | Miss rate |",
        "|---|---:|---:|---:|---:|",
    ]
...
def evaluation_to_html(evaluation: PredictionEvaluation) -> str:
    rows = "".join(
        ...
        f"<td>{item.mean_min_fde_m:.3f} m</td><td>{item.miss_rate:.1%}</td></tr>"
        for item in sorted(
...
<p>Mean minADE {evaluation.mean_min_ade_m:.3f} m · Mean minFDE {evaluation.mean_min_fde_m:.3f} m · Miss rate {evaluation.miss_rate:.1%}</p></section>
<section class="card"><table><thead><tr><th>Scenario</th><th>Agents</th><th>Mean minADE</th><th>Mean minFDE</th><th>Miss rate</th></tr></thead><tbody>{rows}</tbody></table></section>
